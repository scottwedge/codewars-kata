...
def delete_nth(order,max_e):
    # Use dictionary to ensure number of occurrences does not exceed max value
    # Append value to new list unless it exceeds max value
    
    new_list = []
    counts = dict()  # Init counts dictionary
    for j in order:
        if j in counts:
            counts[j] = counts[j] + 1  # Increment
        else:
            counts[j] = 1
